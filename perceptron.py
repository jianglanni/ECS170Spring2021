    ...
    while lap <= max_laps:
        ...
        for i in range(0, len(examples)):
            ...
            for j in range(0, size):
                if examples[i][1][j] == 1:
                    sum_value += weights[j]
            result = sum_value > threshold
            if result and not examples[i][0]:
                for k in range(0, size):
                    if examples[i][1][k] == 1:
                        weights[k] -= adjust
            elif not result and examples[i][0]:
                for k in range(0, size):
                    if examples[i][1][k] == 1:
                        weights[k] += adjust
...
perceptron(0.5, 0.1, [-0.5, 0, 0.5, 0, -0.5], learning1, 4)
perceptron(0.4, 0.09, [0.3, -0.6], learning2, 10)