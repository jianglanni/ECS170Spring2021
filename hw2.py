...
def m_block(state_str):
    ...
    vehicle_block = 0
    for i in range(0, 6)[::-1]:
        if state_str[coord(2, i)] == 'X':
            return vehicle_block + 1  # X car detected, return the value
        elif state_str[coord(2, i)] != '-':
            vehicle_block += 1  # vehicle detected
            if state_str[coord(2, i)] == state_str[coord(1, i)] and state_str[coord(2, i)] == state_str[coord(3, i)]:
                vehicle_block += 1
            else:
                if state_str[coord(2, i)] == state_str[coord(1, i)] and state_str[coord(2, i)] == state_str[coord(0, i)]:
                    vehicle_block += 2
    assert 0  # No X car found, escape
...
def rushhour_astar(heu, state_str):
    ...
    while len(frontier):
        ...
        if is_goal(state_tuple[2]):
            return state_tuple[2]
        visited.append(state_tuple[2])  # Mark as visited
